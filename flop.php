...
class PersonalData {
  ...
  public function setCpr() {
    ...
    $cpr = $this->generateNumber(['iteration'=>1, 'from'=>1 , 'to'=>31]) . $this->generateNumber(['iteration'=>1, 'from'=>1 , 'to'=>12]) . $this->generateNumber(['iteration'=>1, 'from'=>1930 , 'to'=>2022]) ;
    $this->cpr = $cpr;
  ...
  public function setFullNameGenderDate() {
    ...
    if (!preg_match('/^([12]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]))$/', $cpr)) {
      return false;
    } else {
  ...
  public function getAllForOne()                     {  isset($this->firstName) ? $this->firstName : 'yes'; isset($this->lastName) ? $this->lastName : 'yes';  isset($this->gender) ? $this->gender : 'yes'; isset($this->cpr) ? $this->cpr : 'yes';  isset($this->date) ? $this->date : 'yes'; isset($this->adress) ? $this->adress : 'yes'; isset($this->phoneNumber) ? $this->phoneNumber : 'yes'; 
    return <<<PERSON
    <b>Full name</b>: {$this->firstName} {$this->lastName}<br>
    <b>Gender</b>: {$this->gender} <br>
    <b>Date</b>: {$this->date} <br>
